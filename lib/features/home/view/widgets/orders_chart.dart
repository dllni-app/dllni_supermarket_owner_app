    ...
    return BlocBuilder<HomeBloc, HomeState>(
      ...
      builder: (context, state) {
        if (state.dailyCountStatus == BlocStatus.loading) {
          ...
          final days = [
            state.dailyCount!.data!.friday!,
            state.dailyCount!.data!.thursday!,
            state.dailyCount!.data!.wednesday!,
            state.dailyCount!.data!.tuesday!,
            state.dailyCount!.data!.monday!,
            state.dailyCount!.data!.sunday!,
            state.dailyCount!.data!.saturday!,
          ];

          // Total orders this week (sum of pending across all days)
          final totalOrders = days.fold<int>(
            0,
            (sum, day) => sum + day.pending! + day.preparing! + day.completed!,
          );

          // Max Y across ALL values (pending, preparing, completed) for proper scaling
          final allValues = days.expand(
            (day) => [day.pending!, day.preparing!, day.completed!],
          );
          final maxY = allValues.reduce(max).toDouble();
              ...
              children: [
                ...
                Expanded(
                  child: BarChart(
                    BarChartData(
                      alignment: BarChartAlignment.spaceAround,
                      maxY: maxY == 0 ? 10 : maxY * 1.2, // 20% headroom
                      barTouchData: BarTouchData(enabled: false),
                      borderData: FlBorderData(show: false),
                      gridData: FlGridData(
                      ...
                      // 3 bars per group — groupsSpace controls gap between groups
                      groupsSpace: 12,
                      barGroups: List.generate(7, (index) {
                        final day = days[index];
                        return BarChartGroupData(
                          x: index,
                          barsSpace: 4, // gap between the 3 bars inside a group
                          barRods: [
                            BarChartRodData(
